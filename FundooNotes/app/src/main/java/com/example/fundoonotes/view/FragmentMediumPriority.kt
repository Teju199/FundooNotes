...
class FragmentMediumPriority: Fragment() {
    ...
    override fun onCreateView(
        ...
        val view: View = inflater.inflate(R.layout.fragment_medium_priority, container, false)
        val backBtn : ImageView = view.findViewById(R.id.backButton5)

        fstore = FirebaseFirestore.getInstance()
        userID = FirebaseAuth.getInstance().currentUser?.uid.toString()
        notesViewList = view.findViewById(R.id.mediumNotesView)
        noteList = ArrayList()

        displayMediumPriorityNotes()
    ...
    private fun displayMediumPriorityNotes() {
        fstore.collection("users").document(userID).collection("notes")
            .whereEqualTo("priority", "medium").get().addOnCompleteListener {
