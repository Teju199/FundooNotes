...
class FragmentLowPriority: Fragment() {
    ...
    override fun onCreateView(
        ...
        val view: View = inflater.inflate(R.layout.fragment_low_priority, container, false)
        val backBtn : ImageView = view.findViewById(R.id.backButton4)

        fstore = FirebaseFirestore.getInstance()
        userID = FirebaseAuth.getInstance().currentUser?.uid.toString()
        notesViewList = view.findViewById(R.id.lowNotesView)
        noteList = ArrayList()

        displayLowPriorityNotes()
    ...
    private fun displayLowPriorityNotes() {
        fstore.collection("users").document(userID).collection("notes")
            .whereEqualTo("priority", "low").get().addOnCompleteListener {
